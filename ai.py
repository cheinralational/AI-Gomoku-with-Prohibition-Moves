    ...
    def calc_score(self):
        ...
        if self.player_first:
            if self.depth % 2 == 0:  # 该轮到玩家出了
                if player_4_num >= 2 and game.jingshou ==0:#双四且无禁手
                    return -90
                if player_4_num >= 2 and game.jingshou ==1:#双四且有禁手,ai逼迫玩家走禁手
                    return 90
                ...
                if ai_4_num >= 2:
                    return 90
                if player_4_num >= 2 and ai_4_num == 0 and game.jingshou ==0:#人手双四且无禁手，判负
                    return -90
                elif player_4_num >= 2 and ai_4_num == 0 and game.jingshou ==1:#人手双四且有禁手，判正
                    return 90
        ...
        if self.player_first:
            if self.depth % 2 == 0:  # 该轮到玩家出了
                if player_3d_num >= 2 and game.jingshou ==0:#双三并且无禁手
                    return -60
                if player_3d_num >= 2 and game.jingshou ==1:#双三并且有禁手，ai逼迫玩家走禁手
                    return 60
                elif ai_3d_num >= 2:
                    return 60
            else:  # 该轮到电脑出了
                if ai_3d_num >= 2:
                    return 60
                if player_3d_num >= 2 and game.jingshou==0:#人手双三并且无禁手，负分数
                    return -60
                elif player_3d_num >= 2 and game.jingshou==1:#人手双三有禁手，正分数
                    return 60
